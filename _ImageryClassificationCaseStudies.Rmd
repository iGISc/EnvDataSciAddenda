...
```{r eval=F, include=F}
library(tmap)
tm_shape(sentFCC) + tm_graticules() + tm_rgb(max.value=max(sentFCC)@ptr$range_max)
```
...
\index{imagery band display}The Sentinel-2 data are in a 12-bit format, ranging as integers from 0 to 4,095, but are preprocessed to represent reflectances at the base of the atmosphere.  
...
library(tmap)
#max(sentRGB)
tm_shape(sentRGB) + tm_graticules() + tm_rgb(max.value=max(sentRGB)@ptr$range_max)
```
...
```{r include=F, eval=F}
FCC = subset(sentRCV,c(7,3,2))
tm_shape(FCC) + tm_graticules() + tm_rgb(max.value=max(FCC)@ptr$range_max)
```
...
rownames(specRCV) <- specRCV[,1]
specRCV <- specRCV[,-1]
library(RColorBrewer)
LCcolors <- brewer.pal(length(unique(trainPolys9$class)),"Set1")
...
title(main="Spectral signatures Sentinel-2 bands, seven land cover classes", 
      font.main=2)
...
ptsRCV <- spatSample(trainPolys6, 1000, method="random")
extRCV <- terra::extract(sentRCV, ptsRCV)
specRCV <- aggregate(extRCV[,-1], list(ptsRCV$class), mean)
rownames(specRCV) <- specRCV[,1]
specRCV <- specRCV[,-1]
...
title(main="Spectral signatures, six land-cover classes, RCV", font.main=2)
legend('topleft',rownames(specRCV),cex=0.9,lwd=3,col=LCcolors)
```
...
extRCV <- extract(sentRCV, ptsRCV)[,-1] # REMOVE ID
...
Now we'll make a single SpatRaster showing the vegetation/land cover with the *highest probability* (Figure \@ref(fig:imgCARThighesProb20m)).
...
LC20m
```
```{r imgCARThighesProb20m, fig.height=6, fig.width=6, fig.cap=paste0("CART classification, highest probability class, Sentinel-2 20 m ",imgDate)}
...
We might compare this plot with the two previous -- the 20 m (Figure \@ref(fig:img20mClass)) and 10 m (Figure \@ref(fig:img10mClass)) classifications from 28 June.

```{r img20mClass, fig.height=6.5, fig.width=6, fig.cap="Classification of Sentinel-2 20 m image"}
plot(LC20m, col=LCcolors)
```
```{r img10mClass, fig.height=6.5, fig.width=6, fig.cap="Classification of Sentinel-2 10 m spring and summer images"}
plot(LC10m, col=LCcolors)
```
