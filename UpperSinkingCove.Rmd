# Terrain mapping, caves and water samples in a karst

```{r echo=FALSE}
knitr::opts_chunk$set(include=T,echo=T,fig.show=T,results=T,warning=F,message=F,fig.align='center',out.width="75%")
```

**Sinking Cove, Tennessee** is a karst valley system carved into the Cumberland Plateau, a nice place to explore mapping alternatives employing basemaps and environmental measurement point symbolization, from a hydrologic and geochemical study [@davis1993geomorphology] of karst and cave development (Figures \@ref(fig:CaveCoveCaveEnt), \@ref(fig:SinkingCoveCave), and \@ref(fig:SCCspring)). 

- We'll build static maps in **tmap**'s "plot" mode by creating our own basemap from a hillshade (created with **terra**'s `terrain` function) combined with hypsometric tinting of elevation using the `terrain.color` palette. 
- We'll also employ **maptiles** to download an online basemap into a raster to continue using a static map.
- We'll build interactive maps with tmap's leaflet-based "view" mode, allowing us to zoom in as well as select alternative online basemaps.

```{r CaveCoveCaveEnt, out.width = "75%", fig.cap = "Cave Cove Cave Entrance", echo=F}
knitr::include_graphics(here::here("img", "CaveCoveCaveEntrance.png"))
```

```{r SinkingCoveCave, out.width = "75%", fig.cap = "Sinking Cove Cave", echo=F}
knitr::include_graphics(here::here("img", "SinkingCoveCave.png"))
```

```{r SCCspring, out.width = "75%", fig.cap = "Sinking Cove Cave spring", echo=F}
knitr::include_graphics(here::here("img", "SinkingCoveCaveSteephead.png"))
```

## Creating a basemap from a DEM, using hillshade and terrain shading

```{r include=F}
library(igisci)
library(sf); library(tidyverse); library(tmap)
library(terra)
```

We'll read in a digital elevation model (DEM) derived from USGS data, and use it to build a terrain basemap, using a hillshade and hypsometric coloring with a 24-level terrain.color palette, and start with a map showing the overall Sinking Cove Area (Figure \@ref(fig:areaMap)).

```{r areaMap, fig.cap="Sinking Cove Area map, using terra rasters in tmap"}
tmap_mode("plot")
DEM <- rast(ex("SinkingCove/DEM_SinkingCoveUTM.tif"))
slope <- terrain(DEM, v='slope')
aspect <- terrain(DEM, v='aspect')
hillsh <- shade(slope/180*pi, aspect/180*pi, angle=40, direction=330)
# Need to crop a bit since grid north != true north 
bbox0 <- st_bbox(DEM)
xrange <- bbox0$xmax - bbox0$xmin
yrange <- bbox0$ymax - bbox0$ymin
bbox1 <- bbox0
crop <- 0.05
bbox1[1] <- bbox0[1] + crop * xrange # xmin
bbox1[3] <- bbox0[3] - crop * xrange # xmax
bbox1[2] <- bbox0[2] + crop * yrange # ymin
bbox1[4] <- bbox0[4] - crop * yrange # ymax
bboxPoly <- bbox1 %>% st_as_sfc() # makes a polygon
#
tm_shape(hillsh, bbox=bboxPoly) +
  tm_raster(palette="-Greys",legend.show=F,n=20) +
  tm_shape(DEM) +
  tm_raster(palette=terrain.colors(24), alpha=0.5) +
  tm_graticules(lines=F)
```

First we'll prep some data on the caves and water samples.

```{r DataPrep, results="hide"}
library(igisci); library(terra)
library(sf); library(tidyverse); library(readxl); library(tmap)
caves <- st_read(ex("SinkingCove/caves.shp"))
ents <- st_read(ex("SinkingCove/CaveEntrances.shp"))
geology <- st_read(ex("SinkingCove/geology.shp"))
wChemData <- read_excel(ex("SinkingCove/SinkingCoveWaterChem.xlsx")) %>%
  mutate(siteLoc = str_sub(Site,start=1L, end=1L))
wChemTrunk <- wChemData %>% filter(siteLoc == "T") %>% mutate(siteType = "trunk")
wChemDrip <- wChemData %>% filter(siteLoc %in% c("D","S")) %>% mutate(siteType = "dripwater")
wChemTrib <- wChemData %>% filter(siteLoc %in% c("B", "F", "K", "W", "P")) %>% 
  mutate(siteType = "tributary")
wChemData <- bind_rows(wChemTrunk, wChemDrip, wChemTrib)
sites <- read_csv(ex("SinkingCove/SinkingCoveSites.csv"))
wChem <- wChemData %>%
  left_join(sites, by = c("Site" = "site")) %>%
  st_as_sf(coords = c("longitude", "latitude"), crs = 4326)
DEM <- rast(ex("SinkingCove/DEM_SinkingCoveUTM.tif"))   
slope <- terrain(DEM, v='slope')
aspect <- terrain(DEM, v='aspect')
hillsh <- shade(slope/180*pi, aspect/180*pi, 40, 330) 
bounds <- st_bbox(wChem)
tmap_mode("plot")
xrange <- bounds$xmax - bounds$xmin
yrange <- bounds$ymax - bounds$ymin
xMIN <- as.numeric(bounds$xmin - xrange/10)
xMAX <- as.numeric(bounds$xmax + xrange/10)
yMIN <- as.numeric(bounds$ymin - yrange/10)
yMAX <- as.numeric(bounds$ymax + yrange/10)
newbounds <- st_bbox(c(xmin=xMIN, xmax=xMAX, ymin=yMIN, ymax=yMAX), crs= st_crs(4326))
```


Then we'll zoom into upper Sinking Cove, where we have some significant caves (Figure \@ref(fig:upperSinkingCoveCaves)).

```{r upperSinkingCoveCaves, fig.cap="Upper Sinking Cove caves and entrances"}
tm_shape(hillsh,bbox=newbounds) +
  tm_raster(palette="-Greys",legend.show=F,n=20) +
  tm_shape(DEM) + tm_raster(palette=terrain.colors(24), alpha=0.5,legend.show=F) +
  tm_shape(caves) + tm_lines(lwd=2) + tm_text("CaveLabel", size=0.5, just="left", xmod=0.75) +
  tm_shape(ents) + tm_symbols(col="Formation", shape=24, size=0.33, alpha=.75, title.col ="Geologic unit of entrance") + 
  #tm_shape(wChem) + tm_symbols(size="TH", col="Lithology", scale=2, shape="siteType", alpha=1) +
  tm_layout(legend.position = c("left", "bottom")) +
  tm_graticules(lines=F)

```

Or the same map on Geology (Figure \@ref(fig:upperSinkingCoveGeology))
```{r upperSinkingCoveGeology, fig.cap="Upper Sinking Cove geology with caves"}
geol_bounds <- st_bbox(geology)
tm_shape(hillsh,bbox=geol_bounds) +
  tm_raster(palette="-Greys",legend.show=F,n=20) +
  tm_shape(geology) + tm_fill(col="ElevGeol", alpha=0.5, title="Geology by base elevation (m)", legend.reverse=T) +
  #tm_shape(DEM) + tm_raster(palette=terrain.colors(24), alpha=0.5,legend.show=F) +
  tm_shape(caves) + tm_lines(lwd=2) + tm_text("CaveLabel", size=0.5, just="left", xmod=0.75) +
  tm_shape(ents) + tm_symbols(col="Formation", shape=24, size=0.33, alpha=.75, title.col ="Geologic unit of entrance") + 
  #tm_shape(wChem) + tm_symbols(size="TH", col="Lithology", scale=2, shape="siteType", alpha=1) +
  tm_layout(legend.position = c("left", "bottom")) +
  tm_graticules(lines=F)

```

We've studied the water chemistry in cave passages and surface water features to look at limestone solution processes (Figure \@ref(fig:upperSinkingCove)).

```{r upperSinkingCove, warning=F, message=F, fig.cap="Upper Sinking Cove water sample results, with major caves as black lines"}
tm_shape(hillsh,bbox=geol_bounds) +
  tm_raster(palette="-Greys",legend.show=F,n=20) +
  #tm_shape(DEM) + tm_raster(palette=terrain.colors(24), alpha=0.5,legend.show=F) +
  tm_shape(caves) + tm_lines(lwd=2) +
  tm_shape(wChem) + tm_symbols(size="TH", col="navy", border.col="black", scale=2, shape="siteType", alpha=0.66) +
  tm_shape(geology) + tm_fill(col="ElevGeol", alpha=0.33, title="Geology by base elevation (m)", legend.reverse=T) +
  tm_layout(legend.position = c("left", "bottom")) +
  tm_graticules(lines=F)
```

## Using maptiles for a topographic basemap with contours

Using maptiles with tmap in plot mode may produce the most informative map. We could have created contours from the DEM, but it's easier to use a basemap, as long as the internet connection works. The `Esri.WorldTopoMap` in employing USGS labels works well in the US, though elevations are in feet (Figure \@ref(fig:mapTiles)).
```{r mapTiles, out.width="100%", fig.cap="Upper Sinking Cove static map with maptiles"}
library(maptiles)
upperSinkingCove <- get_tiles(wChem, provider="Esri.WorldTopoMap", forceDownload=T)
tm_shape(upperSinkingCove) + 
  tm_rgb() +
  tm_shape(caves, bbox=bounds) + tm_lines(lwd=2) +
  tm_shape(wChem) + 
  tm_symbols(size="TH", col="Lithology", scale=2, shape="siteType", alpha=0.5) +
  tm_layout(legend.position = c("left", "bottom")) +
  tm_graticules(lines=F)
```


## Interactive map with tmap view mode

Interactive map with multiple choices of basemaps, showing water sample site types. This heavily forested area is a good example of why imagery may not be as useful as topographic symbology for a basemap.

```{r waterSampleSites, fig.cap="Upper Sinking Cove interactive map"}
tmap_mode("view")
bounds <- st_bbox()
wChem2map <- filter(wChem, Month == 8)
minVal <- min(wChem2map$TH); maxVal <- max(wChem2map$TH)
tmap_options(basemaps=c(OpenTopoMap="OpenTopoMap", 
                        Esri.WorldTopoMap="Esri.WorldTopoMap",
                        Esri.WorldImagery = "Esri.WorldImagery",
                        Esri.NatGeoWorldMap="Esri.NatGeoWorldMap"))
bounds <- st_bbox(wChem2map)
tm_basemap() +
  tm_shape(caves, bbox=bounds) + tm_lines(lwd=2) +
  tm_shape(wChem2map) + tm_symbols(col="siteType", size="TH", scale=2, alpha=0.5) +
  tm_layout(title=paste("Total Hardness ",as.character(minVal),"-",as.character(maxVal)," mg/L", sep=""))
```

```{r Lithology, eval=F, include=F, fig.cap="Interactive map with lithology"}
tmap_options(basemaps=c(Esri.WorldTopoMap="Esri.WorldTopoMap",
                        Esri.WorldImagery = "Esri.WorldImagery",
                        Esri.NatGeoWorldMap="Esri.NatGeoWorldMap",
                        OpenTopoMap="OpenTopoMap"))
bounds <- st_bbox(wChem2map)
tm_basemap() + 
  tm_shape(caves, bbox=bounds) + tm_lines(lwd=2) +
  tm_shape(wChem2map) + tm_symbols(col="Lithology", size="TH", scale=2, alpha=0.5) +
  tm_layout(title=paste("Total Hardness ",as.character(minVal),"-",as.character(maxVal)," mg/L", sep=""))
```

