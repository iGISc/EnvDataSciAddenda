...
To get it published at https://connect.posit.cloud from a GitHub repo, you'll need an account. Note you'll need at least a basic paid level to do many things, including publishing from a private GitHub repo.

The method I follow now is to go with static HTML document accessing the `_book/index.html` built in RStudio on my computer with bookdown followed by updating the GitHub repo with GitHub desktop.  To update after any changes, I've needed to use the 3-dot menu for the book in https://connect.posit.cloud/jerryddavis and then specify Republish.  
